...
js = """(async function(){
  ...
  try {
    ...
    var api = m.d();
    out.recognizeConfig = api.recognizeConfig.value;
    out.onRecognizeType = typeof api.onRecognize.value;
    // 题目数据 (从 oral.js 相关)
    return JSON.stringify(out);
  } catch(e) { return 'ERR:' + String(e).substring(0,300); }
})()"""
try:
    val = cdp_eval(ws, js)
    print("=== 识别配置 ===", flush=True)
